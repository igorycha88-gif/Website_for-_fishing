...
@router.post(
    ...
    dependencies=[Depends(RateLimiter(times=10, seconds=3600))] if settings.RATE_LIMIT_ENABLED else []
)
async def register(request: RegisterRequest, db: AsyncSession = Depends(get_db)):
    ...
    if existing_email:
        logger.warning(
            "Registration failed - email already exists", email=request.email
        )
        raise HTTPException(
    ...
    if existing_username:
        logger.warning(
            "Registration failed - username already exists", username=request.username
        )
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={
...
@router.post(
    ...
    dependencies=[Depends(RateLimiter(times=5, seconds=60))] if settings.RATE_LIMIT_ENABLED else []
)
async def verify_email(request: VerifyEmailRequest, db: AsyncSession = Depends(get_db)):
    ...
    jti = refresh_token_payload.get("jti") if refresh_token_payload else str(uuid.uuid4())
...
@router.post(
    ...
    response_model=MessageResponse,
    dependencies=[Depends(RateLimiter(times=3, seconds=3600))] if settings.RATE_LIMIT_ENABLED else []
)
async def request_password_reset(
    ...
    user = await user_crud.get_by_email(request.email)
    if not user:
        logger.warning("Password reset failed - email not found", email=request.email)
        return MessageResponse(
    ...
    logger.info(
        "Password reset token generated", user_id=str(user.id), email=request.email
    )
    return MessageResponse(
...
async def confirm_password_reset(
    request: ResetPasswordConfirm, db: AsyncSession = Depends(get_db)
):
    ...
    user_id = payload.get("sub")
    if not user_id:
        logger.warning("Password reset failed - invalid token payload")
        raise HTTPException(
    ...
    if not user:
        logger.warning("Password reset failed - user not found", user_id=user_id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="User not found"
        )

    password_hash = get_password_hash(request.new_password)
    await user_crud.update_password(user_id, password_hash)
    
    await user_crud.increment_token_version(user_id)
    logger.info("Password reset successful, token version incremented", user_id=user_id)
