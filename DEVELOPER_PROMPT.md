...
[ ] 4. Бэкенд: **structlog-логирование** на каждом endpoint (Правило 9)
[ ] 5. Фронтенд: UI компоненты
[ ] 6. Фронтенд: **Unit тесты** (jest)
[ ] 7. **E2E-тесты Playwright** для каждого сценария из ЧТЗ (Правило 8)
[ ] 8. **ВСЕ тесты проходят успешно** (pytest + jest + playwright) (КРИТИЧЕСКИ ВАЖНО!)
[ ] 9. Интеграционное тестирование
[ ] 10. Документация
[ ] 11. **🔄 Перезапуск Docker и всех сервисов** (КРИТИЧЕСКИ ВАЖНО!)
[ ] 12. **🔄 Проверка health checks** (КРИТИЧЕСКИ ВАЖНО!)
```
...
- [ ] 4. Бэкенд: **structlog-логирование добавлено** на каждый endpoint (Правило 9)
...
- [ ] 9. **E2E-тесты Playwright написаны** для каждого сценария из ЧТЗ (Правило 8)
- [ ] 10. **ВСЕ тесты проходят успешно** (pytest + jest + playwright) - КРИТИЧЕСКИ ВАЖНО!
- [ ] 11. **Логи видны в Kibana/stdout**: проверка отправки логов
- [ ] 12. Функциональное тестирование: проверен полный flow
- [ ] 13. Документация обновлена
- [ ] 14. **🔄 ЗАВЕРШЕНИЕ: Перезапуск Docker и всех сервисов** - КРИТИЧЕСКИ ВАЖНО!
- [ ] 15. **🔄 ЗАВЕРШЕНИЕ: Проверка health checks всех сервисов** - КРИТИЧЕСКИ ВАЖНО!

### Чек-лист перед коммитом:
- [ ] Задача согласована с пользователем перед разработкой
- [ ] Unit тесты написаны (backend + frontend)
- [ ] **E2E-тесты Playwright написаны** для сценариев из ЧТЗ (Правило 8)
- [ ] **structlog-логирование добавлено** на каждый backend endpoint (Правило 9)
- [ ] **ВСЕ тесты проходят успешно** (pytest + jest + playwright) (нет падающих тестов)
- [ ] Docker перезапущен: `down && build && up -d`
- [ ] Docker-compose успешно собирается
- [ ] Health checks работают
- [ ] Нет конфликтов портов
- [ ] Логи видны в Kibana/stdout
...
5. **ВСЕГДА** пишем E2E-тесты Playwright для каждого сценария из ЧТЗ (Правило 8)
6. **ВСЕГДА** добавляем structlog-логирование на каждый backend endpoint (Правило 9)
7. **ВСЕГДА** все тесты должны проходить успешно (pytest + jest + playwright) — разработка не завершена, если есть падающие тесты
8. **ВСЕГДА** проверяем логи в Kibana/stdout (INFO для действий, ERROR для ошибок)
9. **ВСЕГДА** используем кэширование для внешних API (Redis с TTL)
10. **ВСЕГДА** реализуем обработку ошибок внешних API (retries, fallback, graceful degradation)
11. **ВСЕГДА** проверяем health checks после перезапуска
12. **ВСЕГДА** перезапускаем Docker и все сервисы в окончании разработки
13. **ВСЕГДА** проверяем логи Docker на ошибки ПЕРЕД и ПОСЛЕ изменений
14. **ВСЕГДА** используем имена сервисов Docker (не localhost) в конфигурации
15. **ВСЕГДА** проверяем совпадение типов данных между Frontend и Backend (List[UUID] ≠ FishType[])

**Процесс разработки (обязательный):**
```
1. Согласование → 2. Разработка → 3. Unit тесты (код) → 4. E2E тесты (Playwright) → 5. structlog-логирование → 6. Все тесты проходят → 7. Перезапуск Docker → 8. Health checks OK → ГОТОВО
```
...
| Файл | Назначение |
|------|------------|
| `VERSIONING.md` | Стратегия версионирования (semver, git tags) |
| `BUSINESS_TASK_FORM.md` | Шаблон формы бизнес-задачи |
| `SKILL_SECURITY.md` | Чеклист безопасности (OWASP Top 10) |
| `PIPELINE_PROD.js` | Продакшн-конвейер (деплой на VPS) |
