...
Управление схемой БД осуществляется через [Alembic](https://alembic.sqlalchemy.org/) — единая конфигурация в корне проекта (`alembic/`, `alembic.ini`).

### Покрытие моделями

| Таблица | Сервис | ORM-модель | Alembic autogenerate |
|---------|--------|------------|---------------------|
| users | auth-service | `app.models.user.User` | да |
| email_verification_codes | auth-service | `app.models.email_verification_code.EmailVerificationCode` | да |
| password_reset_codes | auth-service | `app.models.password_reset_code.PasswordResetCode` | да |
| places | places-service | `app.models.place.Place` | да |
| refresh_tokens | auth-service | нет | нет (только в schema.sql) |
| reports | reports-service | нет | нет |
| bookings | booking-service | нет | нет |
| booking_slots | booking-service | нет | нет |
| categories | shop-service | нет | нет |
| products | shop-service | нет | нет |
| orders | shop-service | нет | нет |
| order_items | shop-service | нет | нет |
| ratings | reports-service | нет | нет |

> Таблицы без ORM-моделей продолжают управляться через `schema.sql`. По мере создания моделей для новых сервисов — добавлять импорты в `alembic/env.py`.

### Workflow изменения схемы

1. Изменить ORM-модель в соответствующем сервисе
2. Сгенерировать миграцию: `DATABASE_URL="..." alembic revision --autogenerate -m "описание"`
3. Проверить сгенерированный файл в `alembic/versions/`
4. Применить: `DATABASE_URL="..." alembic upgrade head`
5. Закоммитить модель + миграцию вместе

### Первоначальная настройка на существующей БД

```bash
# Отметить текущую БД как "на последней версии" (без применения миграций)
...
| expires_at | TIMESTAMP | NOT NULL | Дата истечения кода (15 минут) |
| created_at | TIMESTAMP | default NOW() | Дата создания |

### 13. password_reset_codes
Коды сброса пароля (Auth Service)

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| id | UUID | PK, default gen() | Уникальный идентификатор |
| email | VARCHAR(255) | NOT NULL | Email адрес |
| code | VARCHAR(6) | NOT NULL | Код сброса (6 цифр) |
| attempts | INTEGER | default 0 | Количество попыток |
| expires_at | TIMESTAMP | NOT NULL | Дата истечения кода (15 минут) |
| created_at | TIMESTAMP | default NOW() | Дата создания |
