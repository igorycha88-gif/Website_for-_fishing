...
## Tech Stack
 ...
 - **Next.js Rewrites** - API proxy to backend services
...
│   ├── env.py                 # Migration environment (async mode)
...
**Note**: In development, Next.js automatically proxies API requests (`/api/v1/*`) to the corresponding backend services through Next.js rewrites configuration.
...
In development, Next.js uses rewrites to proxy API requests to backend services:
...
This allows the frontend to make API requests using relative paths (`/api/v1/...`) instead of absolute URLs.

### Database Migrations (Alembic)

Проект использует [Alembic](https://alembic.sqlalchemy.org/) для управления миграциями БД. Единая конфигурация в корне проекта обслуживает все микросервисы.

**Требования:** переменная окружения `DATABASE_URL` (формат: `postgresql+asyncpg://user:pass@host:5432/db`).

```bash
# Применить baseline к существующей БД (только при первом подключении Alembic)
...
DATABASE_URL="postgresql+asyncpg://..." alembic history

# Сгенерировать новую миграцию после изменения ORM-моделей
DATABASE_URL="postgresql+asyncpg://..." alembic revision --autogenerate -m "описание изменений"
...
# Сгенерировать SQL без применения (для ревью)
DATABASE_URL="postgresql+asyncpg://..." alembic upgrade head --sql
```

**Архитектура миграций:**
- Единая папка `alembic/` в корне проекта, одна цепочка версий для всей БД
- Async-режим (asyncpg) — дополнительные sync-драйверы не требуются
- `env.py` автоматически импортирует ORM-модели из auth-service и places-service
- Для добавления моделей нового сервиса — обновить импорты в `alembic/env.py`
