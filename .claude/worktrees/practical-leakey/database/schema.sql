...
CREATE TABLE places (
    ...
    status VARCHAR(30) DEFAULT 'active' CHECK (status IN ('active', 'pending_moderation', 'rejected')),
...
-- Password reset codes table (Auth Service)
...
-- Trigger for email_verification_codes updated_at
CREATE TRIGGER update_email_verification_codes_updated_at BEFORE UPDATE ON email_verification_codes
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Trigger for password_reset_codes updated_at
CREATE TRIGGER update_password_reset_codes_updated_at BEFORE UPDATE ON password_reset_codes
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
